"""SPARRCLE project reporting module.
...
-----
CLI:
    mix-models sparrcle report --model MODEL --scenario SCENARIO

Python API:
    from message_ix_models.project.sparrcle.report import (
...
def _get_config_path() -> Path:
    ...
    return package_data_path("sparrcle", "report", "water_cooling.yaml")
...
def prepare_water_reporter(
    ...
    """Prepare a Reporter with SPARRCLE water/cooling computations.
    ...
    if config_path.exists():
        log.info(f"Loading SPARRCLE water/cooling report config from {config_path}")
...
def water_report_callback(rep: Reporter, context) -> None:
    ...
    if config_path.exists():
        rep.configure(path=config_path)
        log.info("Registered SPARRCLE water/cooling reporting")
