...
def test_direct_water_build_call(water_build_scenario):
    ...
    pytest.skip(
        "Direct water build test skipped due to scenario locking issue in add_water_supply"
    )
...
def test_water_supply_vintage_activity_consistency(prepared_scenario):
    ...
    if context.regions == "ZMB":
        pytest.skip(
            "Skipping vintage-activity test for ZMB due to limited water supply support."
        )
    ...
    _apply_and_commit(
        scenario, context, add_water_supply, "test water supply consistency"
    )
    ...
    for tech in ["extract_surfacewater", "extract_groundwater"]:
        ...
        if len(unique_va_sets) > 1:
            ...
            print(
                f"✅ {tech} has consistent vintage-activity years across all {len(vintage_activity_sets)} basin nodes"
            )
...
def test_water_parameter_unit_consistency(prepared_scenario):
    ...
    expected_units = {
        "input": [
            ...
            "-",
            "GWa",
        ],  # Water inputs can be MCM or dimensionless
            ...
            if param_name in ["input", "output"]:
                ...
                for commodity in water_commodities:
                    ...
                    if not param_data.empty and "unit" in param_data.columns:
                        ...
                        if unexpected:
                            issues_found.append(
                                f"{param_name} for {commodity}: unexpected units {unexpected}"
                            )
            else:
                ...
                for tech in water_techs:
                    ...
                    if not param_data.empty and "unit" in param_data.columns:
                        ...
                        if unexpected:
                            issues_found.append(
                                f"{param_name} for {tech}: unexpected units {unexpected}"
                            )
        except Exception as e:
...
def test_no_duplicate_parameters(prepared_scenario):
    ...
    if duplicate_stats:
        ...
        for param, stats in duplicate_stats.items():
            print(
                f"   {param}: {stats['duplicates']} duplicates out of {stats['total']} rows ({stats['duplicate_pct']:.1f}%)"
            )

        # Flag if any parameter has more than 10% duplicates
        high_duplicate_params = [
            p for p, s in duplicate_stats.items() if s["duplicate_pct"] > 10
        ]
        if high_duplicate_params:
            assert False, (
                f"High duplicate rate (>10%) in parameters: {high_duplicate_params}"
            )
    else:
    ...
    for tech in irrigation_techs:
        ...
        if missing_active_years:
            coverage_issues.append(
                f"{tech} missing activity in years: {sorted(missing_active_years)}"
            )
        ...
        if missing_vintage_years:
            coverage_issues.append(
                f"{tech} missing vintages for years: {sorted(missing_vintage_years)}"
            )

        print(
            f"   {tech}: {len(vintage_years)} vintages, {len(active_years)} active years"
        )
...
def test_water_balance_units(prepared_scenario):
    ...
    water_units = [
        u for u in unit_groups.keys() if any(x in u for x in ["MCM", "km3", "km^3"])
    ]
    ...
    if len(water_units) > 1:
        ...
        print(
            f"✅ Water balance uses consistent units: {water_units[0] if water_units else 'No water units found'}"
        )
...
def test_water_supply_value_validation(prepared_scenario):
    ...
    sw_input = scenario.par(
        "input", {"technology": "extract_surfacewater", "commodity": "electr"}
    )
    if not sw_input.empty:
        ...
        if abs(sw_energy - sw_range["typical"]) < 0.001:
            print(
                f"✅ Surface water energy matches expected value: {sw_range['typical']}"
            )
        else:
            print(
                f"⚠️  Surface water energy differs from typical value {sw_range['typical']}"
            )

    # Groundwater electricity consumption
    gw_input = scenario.par(
        "input", {"technology": "extract_groundwater", "commodity": "electr"}
    )
    ...
    for tech, lifetime_key in lifetime_mapping.items():
        ...
        if not tl_data.empty:
            ...
            expected_lifetime = ranges["water_supply"]["technical_lifetimes"][
                lifetime_key
            ]
            ...
            print(
                f"✅ {tech} lifetimes match expected value: {expected_lifetime} years"
            )
...
def test_infrastructure_value_validation(prepared_scenario):
    ...
    context["water build info"] = info
    _apply_and_commit(
        scenario, context, add_infrastructure_techs, "test infrastructure values"
    )

    # Test 1: Infrastructure investment costs
    print("\n--- Testing Infrastructure Investment Costs ---")

    infra_cost_mapping = {"urban_t_d": "urban_td", "rural_t_d": "rural_td"}
    ...
    for tech in ["urban_t_d", "rural_t_d"]:
        ...
        if not elec_input.empty:
            ...
            elec_range = ranges["infrastructure"]["electricity_consumption"][
                tech.replace("_t_d", "_td")
            ]
