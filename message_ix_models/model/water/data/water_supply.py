...
def map_basin_region_wat(context: "Context") -> pd.DataFrame:
    ...
    match context.time:
        case "year" | 'year'| ['year']: # ['year'] is used in context.time when called from water for ppl.
...
def _process_cooling_supply(context: "Context", year_wat: tuple, node_region: Any) -> dict[str, pd.DataFrame]:
    """Processes rules specific to the 'cooling' nexus setting."""
    results: dict[str, pd.DataFrame] = {}
    cooling_outputs = []
    dummy_df = pd.DataFrame()
    base_cooling = {"rule_dfs":
    {"df_node": dummy_df,
    "runtime_vals": {"year_wat": year_wat}
    },
    "node_loc": node_region}  # no dfs used in cooling rules
