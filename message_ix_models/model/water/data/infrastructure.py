...
def add_desalination(context: "Context") -> dict[str, pd.DataFrame]:
    ...
    # Forward-looking lower bound on desal activity from historical capacity:
    # for the first three model periods, force at least the carry-forward 2025
    # historical addition rate through each desal tech.
    ...
    # Restrict to model years: pre-firstmodelyear ACT is governed by
    # historical_new_capacity and would conflict at basins where historical
    # CAP is below the 2025 addition rate.
    bound_lo = bound_lo[
        (bound_lo["year_act"] >= firstyear)
        & (bound_lo["year_act"] <= firstyear + 15)
    ]
    bound_lo["value"] = bound_lo["value"] / ANNUAL_CAPACITY_FACTOR

    # Membrane and distillation share extract_salinewater_basin upstream.
    # The previous per-tech min(value, cap) clip allowed sum(bound_lo) > cap
    # because each tech was capped independently; scale the per-tech floors
    # down in proportion so the basin-year sum respects the extraction cap.
